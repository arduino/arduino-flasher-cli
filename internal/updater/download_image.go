...
func DownloadAndExtract(ctx context.Context, targetVersion string, boardType string, os string, temp *paths.Path) (string, string, error) {
	tmpZip, version, board, err := DownloadImage(ctx, targetVersion, boardType, os, temp)
	if err != nil {
...
func DownloadImage(ctx context.Context, targetVersion, boardType, os string, downloadPath *paths.Path) (*paths.Path, string, string, error) {
	...
	callback := func(current, total int64) {
		if bar == nil {
			bar = progressbar.DefaultBytes(
				total,
				i18n.Tr("Downloading %s image version %s", rel.OS, rel.Version),
			)
		}
		_ = bar.Set64(current)
	}
	if tmpZip, err := client.DownloadFile(ctx, downloadPath, rel, callback); err != nil {
		return nil, "", "", fmt.Errorf("could not download %s image: %w", rel.OS, err)
	} else {
		return tmpZip, rel.Version, rel.Board, nil
	}
}
