...
// directly instead.
func DownloadAndFlash(ctx context.Context, ref registry.ImageRef, opts FlashOptions) error {
	...
	version, err := DownloadAndExtract(ctx, ref, temp)
	if err != nil {
		return fmt.Errorf("could not download and extract the image: %v", err)
	}

	return FlashBoard(ctx, opts.Serial, temp, version, ref.Board, opts.PreserveUser, opts.RootSize, nil)
}
...
func checkFreeSpace(dir *paths.Path, requiredGiB uint64, what string) error {
	...
	if d.Free/GiB < requiredGiB {
		return fmt.Errorf("%s requires up to %d GiB of free space", what, requiredGiB)
	}
	return nil
}
