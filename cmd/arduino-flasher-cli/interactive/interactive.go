...
func Run(ctx context.Context) {
	...
	// Only the UNO Q for now: a second board would need asking which one, and a
	// programmer it accepts for the capacity read.
	boards := registry.BoardsWithImages()
	if len(boards) != 1 || boards[0].Board != registry.UnoQ {
		feedback.Fatal(i18n.Tr("the interactive mode supports the %s only: name a board on the command line instead", registry.UnoQ), feedback.ErrBadArgument)
	}
	...
	sp := spinner.New().
		...
		ActionWithErr(func(ctx context.Context) error {
			index, err := registry.IndexFor(board, "")
			if err != nil {
				return err
			}
	...
	// The board cannot be read yet: that needs a programmer, and the only one
	// available before a download is the embedded one, which is not necessarily
	// the right one for the board. The sizes are checked against the real GPT
	// once the image has been extracted.
	variantOptions := make([]huh.Option[registry.Variant], 0, len(boards[0].Variants))
	for _, v := range boards[0].Variants {
	...
	form := huh.NewForm(
		...
		huh.NewGroup(
			huh.NewSelect[registry.Variant]().
				Title(i18n.Tr("Select your %s", board)).
	...
	ref := registry.ImageRef{Version: selectedVersion, Board: board}
	if err := updater.DownloadAndFlash(ctx, ref, updater.FlashOptions{PreserveUser: preserveUser, RootSize: rootSize}); err != nil {
